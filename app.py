...
# ── PÄIVITETYT URHEILU- JA KULTTUURI-SKRAPPAAJAT ──────────────────────────────

@st.cache_data(ttl=600) # Lyhennetty välimuisti
def hae_liiga_tanaan():
    try:
        ...
        resp = requests.get(f"https://liiga.fi/api/v2/games?tournament=SM-liiga&season={kausi}", headers=headers, timeout=10)
        resp.raise_for_status()
...
def hae_ooppera_tanaan():
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        # Korjattu tarkkaan päivämäärämuotoon, esim. 13.3.2026
        pvm_str = f"{now.day}.{now.month}.{now.year}"
        # Oikean selaimen tiedot ohittamaan estot
        ...
        if pvm_str in resp.text:
            return [f"Näytös löytyi sivuilta ({pvm_str})"]
        return []
    except:
        return []
...
def hae_hkt_tanaan():
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        # Korjattu tarkkaan päivämäärämuotoon
        pvm_str = f"{now.day}.{now.month}.{now.year}"
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}
        resp = requests.get("https://hkt.fi/ohjelmisto/", headers=headers, timeout=10)
        if pvm_str in resp.text:
            return [f"Esityksiä tänään ({pvm_str})"]
        return []
    except:
        return []
...
def render_dashboard():
    ...
    # UUSI NAPPI: PAINAMALLA TÄTÄ TYHJENNETÄÄN VÄLIMUISTI JA PÄIVITETÄÄN DATA
