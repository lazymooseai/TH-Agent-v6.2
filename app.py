...
# ── UUDET URHEILU- JA KULTTUURI-SKRAPPAAJAT ───────────────────────────────────

@st.cache_data(ttl=3600)
def hae_liiga_tanaan():
    """Hakee Liigan piilotetusta JSON-rajapinnasta tämän päivän pelit"""
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        kausi = now.year if now.month > 6 else now.year - 1
        resp = requests.get(f"https://liiga.fi/api/v2/games?tournament=SM-liiga&season={kausi}", timeout=10)
        resp.raise_for_status()
        tanaan_str = now.strftime("%Y-%m-%d")
        pelit = []
        for peli in resp.json():
            if peli.get("start", "").startswith(tanaan_str):
                koti = peli.get("homeTeam", {}).get("teamName", "Koti")
                vieras = peli.get("awayTeam", {}).get("teamName", "Vieras")
                aika = peli.get("start", "")[11:16]
...
@st.cache_data(ttl=3600)
def hae_ooppera_tanaan():
    """Lukee nopeasti Oopperan sivuilta onko siellä mainittu tämän päivän päivämäärä"""
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        pvm_str = f"{now.day}.{now.month}."
        resp = requests.get("https://oopperabaletti.fi/ohjelmisto/", timeout=10)
        soup = BeautifulSoup(resp.text, 'html.parser')
        if pvm_str in soup.text:
            return ["Esitys varmistettu! (Tarkista ohjelmisto)"]
        return []
    except:
        return []
...
@st.cache_data(ttl=3600)
def hae_hkt_tanaan():
    """Varmistus HKT:n sivuilta, jos API ei toimi"""
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        pvm_str = f"{now.day}.{now.month}." 
        resp = requests.get("https://hkt.fi/ohjelmisto/", timeout=10)
        if pvm_str in resp.text:
            return ["Näytöksiä tänään (Tarkista ohjelmisto)"]
        return []
    except:
        return []

# Parannettu API-haku: Haetaan 200 tapahtumaa pelkän 20 sijaan!
@st.cache_data(ttl=3600)
def get_culture_live_events():
    live_tapahtumat = {}
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        # Rajoitetaan loppuaika tämän päivän loppuun ja lisätään limit=200
        alku = now.replace(hour=0, minute=0, second=0).isoformat()
        loppu = now.replace(hour=23, minute=59, second=59).isoformat()
...
# Päivitetty yhdistämisfunktio: jos API pettää, käytetään omia varmistusskraappareita
def yhdista_kulttuuridata(paikat):
    live_data = get_culture_live_events()
    ooppera_tanaan = hae_ooppera_tanaan()
    hkt_tanaan = hae_hkt_tanaan()

    for p in paikat:
        hakusanat = p.get("hakusanat", [])
        tapahtumat = []

        # 1. Etsitään kaupungin API:sta
        ...
        # 2. Jos API ei löydä, käytetään varasuunnitelmia (esim. Oopperalle)
        nimi = p.get("nimi", "").lower()
        if not tapahtumat:
            if "ooppera" in nimi and ooppera_tanaan:
                tapahtumat = ooppera_tanaan
            elif "kaupunginteatteri" in nimi and hkt_tanaan:
                tapahtumat = hkt_tanaan

        if tapahtumat:
            p["lopetus_html"] = f"<span class='live-event'>🟢 LIVE TÄNÄÄN: {' | '.join(tapahtumat)}</span>"
        else:
            if hakusanat: p["lopetus_html"] = f"<span class='no-event'>⚪ Ei tunnettua esitystä tänään.</span><br><span style='color:#777;'>Tyypillisesti: {p['lopetus']}</span>"
            else: p["lopetus_html"] = f"<span class='endtime'>⏱ Tyypillinen lopetus: {p['lopetus']}</span>"
    return paikat

# Uusi funktio urheilulle
def yhdista_urheiludata(paikat):
    liiga_pelit = hae_liiga_tanaan()
    for p in paikat:
        nimi = p.get("nimi", "").lower()
        tapahtumat = []
        
        # Etsitään päivän Liiga-peleistä HIFK tai K-Espoo
        if "hifk" in nimi:
            for peli in liiga_pelit:
                if "HIFK" in peli["koti"]:
                    tapahtumat.append(f"HIFK - {peli['vieras']} (klo {peli['aika']})")
        elif "kiekko-espoo" in nimi:
            for peli in liiga_pelit:
                if "K-Espoo" in peli["koti"]:
                    tapahtumat.append(f"K-Espoo - {peli['vieras']} (klo {peli['aika']})")
        
        if tapahtumat:
            p["lopetus_html"] = f"<span class='live-event'>🏒 PELI TÄNÄÄN: {' | '.join(tapahtumat)}</span><br><span style='color:#ccc;font-size:15px;'>Kesto n. 2,5h aloituksesta</span>"
        else:
            p["lopetus_html"] = f"<span class='no-event'>⚪ Ei Liiga-kotiottelua tänään.</span>"
    return paikat
...
def render_dashboard():
    ...
    with tab1:
        ...
        st.markdown(f"<div class='taksi-card'>{venue_html(yhdista_kulttuuridata(kulttuuri_paikat))}</div>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        # Huomaa tämä uusi funktio urheilulle:
        st.markdown(f"<div class='taksi-card'>{venue_html(yhdista_urheiludata(urheilu_paikat))}</div>", unsafe_allow_html=True)
    ...
    with tab5:
        ...
        if general_tapahtumat:
            ...
            for t in general_tapahtumat:
                live_html += f"<div class='eventline'><span class='badge-blue'>LIVE</span> <span class='venue-name'>{t['nimi']}</span><br><span class='venue-address'>📍 {t['paikka']}</span><br><span class='endtime'>⏱ Päättyy: klo {t['loppu']}</span></div>"
